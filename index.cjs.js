...
function getCustomPropertiesFromRoot(root, opts) {
  ...
  const customPropertiesFromGlobalStyle = {}; // for each html or :root or :global rule

  root.nodes.slice().forEach(rule => {
    const customPropertiesObject = isHtmlRule(rule) ? customPropertiesFromHtmlElement : isRootRule(rule) ? customPropertiesFromRootPseudo : isGlobalRule(rule) ? customPropertiesFromGlobalStyle : null; // for each custom property
...
const isGlobalRule = node => node.type === "rule" && node.selector.split(",").some(item => globalSelectorRegExp.test(item)) && Object(node.nodes).length;
...
const creator = opts => {
  ...
  const exportTo = [].concat(Object(opts).exportTo || []); // export equivalent JS file to dir

  const exportDir = "exportDir" in Object(opts) ? opts.exportDir : false; // promise any custom selectors are imported
  ...
  const asyncTransform = async root => {
    ...
    if (exportDir) {
      ...
      rulesByFile.forEach(async file => {
        const fileName = path__default['default'].basename(Object.keys(file)[0], ".css") + ".js";
        const fileRules = Object.values(file)[0];
        const newFileName = path__default['default'].resolve(exportDir, fileName);
